...
需要 Python 3.11+、Git、GitHub CLI（`gh`）與 [uv](https://docs.astral.sh/uv/getting-started/installation/)，以及自己的 TypeSafe API key。

### 1. 取得專案與安裝依賴

使用 GitHub CLI（`gh`）下載專案。已下載專案者直接在專案根目錄執行 `uv sync`。
